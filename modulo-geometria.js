...
var filas=20;
var columnas=20;
var superficie = new TuboSenoidal(0.5,0.3,1,1);//Esfera(1);//Plano(3,3);
...
function Esfera(radio){
    ...
    this.getPosicion=function(u,v){
        uPi = u*2*Math.PI;
        vPi = v*Math.PI;
        var x = radio*Math.cos(uPi)*Math.sin(vPi);
        var y = radio*Math.sin(uPi)*Math.sin(vPi);
        var z = radio*Math.cos(vPi);
    ...
    this.getNormal=function(u,v){
        uPi = u*2*Math.PI;
        vPi = v*Math.PI;
        var x = Math.cos(uPi)*Math.sin(vPi);
        var y = Math.sin(uPi)*Math.sin(vPi);
        var z = Math.cos(vPi);
...
function TuboSenoidal(amplitud, longitud, radioTubo, altura){

    this.getPosicion=function(u,v){
        uPi = u*2*Math.PI;
        vPi = v*2*Math.PI;
        
        var radio =radioTubo + amplitud*Math.sin(vPi/longitud);

        var x = radio*Math.cos(uPi);
        var y = radio*Math.sin(uPi);
        var z = altura*(v-0.5);
    ...
    this.getNormal=function(u,v){
        uPi = u*2*Math.PI;
        vPi = v*2*Math.PI;
        var x = altura*Math.cos(uPi); 
        var y = altura*Math.sin(uPi);
        var z = -(amplitud*2*Math.PI/longitud) *Math.cos(vPi/longitud);
        return [x,y,z];
    }
    this.getCoordenadasTextura=function(u,v){
        return [u,v];
...
function generarSuperficie(superficie,filas,columnas){
    ...
    indexBuffer=[];  
    //indexBuffer=[0,2,1,3]; // Estos valores iniciales harcodeados solo dibujan 2 triangulos, REMOVER ESTA LINEA!
    ...
    for (i=0; i < filas; i++) {
        ...
        indexBuffer.push((i+1)*(columnas+1));
        indexBuffer.push((i+2)*(columnas+1));
        indexBuffer.push((i+2)*(columnas+1));
    }
